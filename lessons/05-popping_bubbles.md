...
Now that we are able to start the game and spawn bouncing bubbles, we need to add a way to pop them. First we need to create a `bubble-popper/src/assets/sfx/` folder. Then we need to download the bubble popping sound effect from https://github.com/DylanCheetah/bubble-popper/blob/9c59bb470f2725d607fbc949f4e36a98b942fbd5/src/assets/sfx/Bubble-Pop.wav and move it to `bubble-popper/src/assets/sfx/`. We also need to download the sewing needle image from https://github.com/DylanCheetah/bubble-popper/blob/main/src/assets/sprites/Needle.png?raw=true and move it to `bubble-popper/src/assets/sprites/`. Next we need to open `bubble-popper/package.json` and modify the build:copy script like this:
```json
